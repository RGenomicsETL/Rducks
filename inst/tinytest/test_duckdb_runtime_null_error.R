...
local({
  ...
  invisible(rducks_register_scalar_udf(con, "rducks_null_default", function(x) stop("should not be called"), "i32", "i32"))
  ...
  invisible(rducks_register_scalar_udf(con, "rducks_i32_na_return", function() NA_integer_, character(), INTEGER))
  expect_true(is.na(DBI::dbGetQuery(con, "SELECT rducks_i32_na_return() AS x")$x))
  invisible(rducks_register_scalar_udf(con, "rducks_i32_nan_bad", function() NaN, character(), INTEGER))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_i32_nan_bad() AS x"), "Rducks")
  invisible(rducks_register_scalar_udf(con, "rducks_i32_inf_bad", function() Inf, character(), INTEGER))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_i32_inf_bad() AS x"), "Rducks")
  invisible(rducks_register_scalar_udf(con, "rducks_i32_fraction_bad", function() 1.5, character(), INTEGER))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_i32_fraction_bad() AS x"), "Rducks")
  invisible(rducks_register_scalar_udf(con, "rducks_i32_length_bad", function() c(1L, 2L), character(), INTEGER))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_i32_length_bad() AS x"), "length 1")
  invisible(rducks_register_scalar_udf(con, "rducks_i32_empty_bad", function() integer(), character(), INTEGER))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_i32_empty_bad() AS x"), "length 1")
  invisible(rducks_register_scalar_udf(con, "rducks_struct_i32_nan_bad", function() list(a = NaN), character(), STRUCT(a = INTEGER)))
  expect_error(DBI::dbGetQuery(con, "SELECT rducks_struct_i32_nan_bad() AS x"), "marshal")
  invisible(rducks_register_scalar_udf(con, "rducks_double_inf_return", function() Inf, character(), DOUBLE))
  expect_true(is.infinite(DBI::dbGetQuery(con, "SELECT rducks_double_inf_return() AS x")$x))
  invisible(rducks_register_scalar_udf(con, "rducks_double_nan_return", function() NaN, character(), DOUBLE))
  expect_true(is.nan(DBI::dbGetQuery(con, "SELECT rducks_double_nan_return() AS x")$x))
  suppressWarnings(invisible(rducks_register_scalar_udf(con, "rducks_float_inf_return", function() Inf, character(), FLOAT)))
  ...
  invisible(rducks_register_scalar_udf(con, "rducks_counter", counter, character(), "i32", side_effects = TRUE))
