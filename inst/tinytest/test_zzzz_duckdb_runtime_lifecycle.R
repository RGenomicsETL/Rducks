...
rducks_runtime_lifecycle_body <- function() {
  ...
  local({
    ...
    expect_true(runtime_stats$native_release_supported[[1L]])
  ...
  local({
    ...
    open_before_release <- rducks_runtime_stats(con)$connections_current[[1L]]
    rducks_release(con)
    open_after_release <- rducks_runtime_stats(con)$connections_current[[1L]]
    expect_true(open_after_release <= open_before_release - 2L)
