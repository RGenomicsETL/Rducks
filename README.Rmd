...
Rducks registers R functions as DuckDB SQL functions through a package-managed
DuckDB C extension. It is built around explicit type descriptors, DuckDB Arrow C
...
Scalar UDFs also choose an evaluation mode (`"scalar"` or `"vectorized"`) and an
execution plan (`arrow_r`, `arrow_c`, or `arrow_ipc`). Aggregates, table
functions, and query streams are separate APIs rather than scalar-UDF plan
variants.
...
concrete argument types and Rducks then materializes those inputs as if the
signature had been declared with `args = ...`. Use `args = NULL` only for a true
zero-argument UDF.
...
Aggregates keep preserved R state owned by the extension and execute on the
recorded R thread.
...
Use `rducks_query_stream()` when an R caller wants explicit DuckDB-native query
batches instead of an eager `DBI::dbGetQuery()` result.
...
For process isolation, choose the Arrow IPC plan. Local providers are launched as
mirai workers and communicate with the extension over generated NNG endpoints.
Explicit `ipc_globals`, `ipc_packages`, and `ipc_globals_share = "mori"` are
available when worker functions need external R objects.
...
argument inspection, and selection-vector helpers. The current detected surface
is:
...
`tools/used_duckdb_unstable_api.R` regenerates this table. See `docs/BUILD.md`
for the build and ABI details.
