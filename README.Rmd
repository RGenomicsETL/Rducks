...
The package builds its DuckDB extension during installation with `configure` or
`configure.win`. Vendored NNG/Mbed TLS support is optional at build time but used
by the native Arrow IPC provider when available. DuckDB C API headers are
refreshed explicitly when the supported DuckDB version changes.
...
connection/runtime access, scalar-function bind/init hooks, and selection-vector
helpers. The current surface is tracked by `tools/used_duckdb_unstable_api.R` and
