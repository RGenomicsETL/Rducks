...
one-R-call-per-DuckDB-chunk semantics as `arrow_r` vectorized mode. If we add a
true parallel `arrow_c` execution variant beyond same-process queued dispatch,
it must get an explicit plan name and support matrix entry rather than being
hidden under `serial`.
...
- [ ] **P2** Implement native `arrow_c + vectorized` as a first-class implementation.
  - Required combinations:
    ...
    - `arrow_c + inproc_concurrent + vectorized` for same-process queued
      concurrent/parallel DuckDB scheduling while keeping R calls on the main R
      lane.
    - a future explicitly named true-parallel native `arrow_c` variant if we can
      prove the hot path has no R API calls and no borrowed DuckDB/R lifetime
      hazards.
