...
- [~] **P3** Plug `multiprocess_parallel` into the same scheduler interface.
  - Current scalar-callback implementation: `arrow_ipc` uses generic `future`
    ...
    collect a group of queued futures after submission. A main-lane RIPC
    callback cooperatively drains queued worker callbacks into the same wave,
    avoiding the single-request timeout path for parallel DuckDB UDF execution.
    Native counters expose `ripc_collect_batches`, `ripc_collect_requests`,
