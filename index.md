...
worker-process (`transport = "ipc"`, native
...
values on the recorded R thread. `next_batch()` returns the next data
frame or `NULL` at end of stream.
...
with [`dbGetQuery()`](https://dbi.r-dbi.org/reference/dbGetQuery.html)),
yet the resident set stays a small, roughly constant fraction of that.
