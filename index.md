...
Rducks normalizes the declared DuckDB type objects, checks that
scalar-mode marshalling is available, and preserves the R callback.
Registration then crosses back through SQL: Rducks calls the extension
function `rducks_register_scalar(...)`, passing the callback reference,
type descriptor tokens, and NULL/exception/side-effect flags. The
extension registers one DuckDB scalar function implementation and stores
the per-UDF metadata in DuckDB `extra_info`. During query execution,
that generic DuckDB callback exports input chunks through the DuckDB
...
and registers scalar-mode R UDFs with
[`rducks_register()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register.md).
The scalar-mode input/output type set is `BOOLEAN`, `TINYINT`,
`UTINYINT`, `SMALLINT`, `USMALLINT`, `INTEGER`, `UINTEGER`, `BIGINT`,
`UBIGINT`, `FLOAT`, `DOUBLE`, `VARCHAR`, `BLOB`, `DATE`, `TIME`,
...
These classes preserve exact representation at the scalar-mode callback
...
Only `scalar` is public and implemented now.
