...
stores preserved R object aggregate state and can call row-wise or
chunk-wise update/combine/finalize callbacks on the recorded R thread.
[`rducks_register_table()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_table.md)
covers finite or streaming scans whose positional SQL argument count is
inferred from the R function formals and whose input types are
registered as DuckDB `ANY`; finite results are imported once during
bind, while
[`rducks_table_stream()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_table_stream.md)
lets scan-time batches be imported one at a time. For R callers that
want query results incrementally,
[`rducks_query_stream()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_query_stream.md)
...
connection, normalized signature, and registration options. You do not
need to keep this object for the scalar UDF to keep working in DuckDB.
...
[`rducks_register_scalar_udf()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_scalar_udf.md)
for DuckDB scalar UDF returns and, when declared, DuckDB scalar UDF
arguments. They are also used by
[`rducks_register_aggregate()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_aggregate.md)
...
and applies to DuckDB scalar UDFs registered with
[`rducks_register_scalar_udf()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_scalar_udf.md).
...
the exported schema that Rducks uses to document DuckDB scalar-UDF
...
[`rducks_register_aggregate()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_aggregate.md)
registers R-backed DuckDB aggregates. The aggregate state is an
arbitrary R object preserved by the extension, not a serialized `raw`
vector. Returning `NULL` means an empty/no-state aggregate; otherwise
update/combine callbacks may return any R object. Aggregates still run
on the recorded R thread and are not controlled by scalar-UDF execution
...
[`rducks_register_table()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_table.md)
registers finite or streaming R-backed table functions. The number of
positional SQL arguments comes from `formals(fun)`, each input is
registered as DuckDB `ANY`, and the output schema is inferred at bind
time. `fun()` may return a data frame, a named list of columns, or
[`rducks_table_stream()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_table_stream.md).
Finite results are imported once during bind; streaming results provide
a prototype plus `next_batch()` and are imported one batch at a time
during scan. Projection pushdown still avoids copying unreferenced
columns into DuckDB output chunks. Register and execute table functions
with `rducks_enable(con, threads = "single")`.
...
[`rducks_table_stream()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_table_stream.md)
with a zero-row prototype and a `next_batch()` callback.
...
temporary tables/views are not part of the stream query scope. That
stream connection is separate from the extension connection used for
dynamic scalar/table/aggregate registration, and a caller connection
currently supports one active native query stream at a time. Delivery
into R is intentionally on the recorded R thread because record-batch
wrapping and optional data-frame materialization create R objects;
Rducks does not call R/nanoarrow code from arbitrary DuckDB worker
threads. The returned stream is connection-bound for lifecycle and has
`next_batch()`, [`close()`](https://rdrr.io/r/base/connections.html),
and `is_closed()` methods. By default `next_batch()` materializes a base
R data-frame batch; with `format = "record_batch"` it returns the owned
...
streams. Registration remains semantic: name, function, Rducks
evaluation mode, types, NULL handling, exception handling, and
side-effect flag. The plan active at
[`rducks_register_scalar_udf()`](https://sounkou-bioinfo.github.io/Rducks/reference/rducks_register_scalar_udf.md)
freezes the scalar UDF’s native evaluator/marshalling metadata; later
plan changes do not retarget already-registered scalar UDFs.
...
to inspect the plan and native counters for a registered scalar UDF.
...
lets DuckDB worker threads enqueue same-process R calls for scalar UDFs
while the recorded main R thread drains the queue. This is for safety
and liveness, not automatic speed: R API work still runs on the main
thread.
...
for the registered scalar UDF lifetime.
