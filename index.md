...
Aggregate functions expose a single aggregate execution path: serialized
R callbacks on the recorded R thread. They do not have
row-wise/vectorized scalar-UDF evaluation modes and are not controlled
by scalar-UDF execution plans. Registration requires
...
table-function execution path: R callbacks on the recorded R thread.
They do not have row-wise/vectorized scalar-UDF evaluation modes and are
not controlled by scalar-UDF execution plans; register and execute them
