...
- Added `rducks_query_stream()` as a connection-bound R-side streaming query
  ...
  export fetched chunks via DuckDB Arrow C Data, and either return owned
  nanoarrow record batches (`format = "record_batch"`) or materialize through
  the existing Rducks/nanoarrow conversion helpers, without requiring the
  `arrow` package.
