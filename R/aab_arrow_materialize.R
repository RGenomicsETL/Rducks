...
rducks_arrow_bool8_array <- function(values, schema) {
  ...
  valid <- !is.na(values)
  # Valid arrow.bool8 values are encoded as 0x00/0x01. Null slots still use
  # the Arrow validity bitmap as authoritative, but keep their data byte as a
  # non-value sentinel so a raw-buffer inspection cannot confuse NULL with
  # FALSE.
  data <- as.raw(ifelse(is.na(values), 255L, ifelse(values, 1L, 0L)))
  array <- nanoarrow::nanoarrow_array_init(schema)
  nanoarrow::nanoarrow_array_modify(
    array,
    list(
