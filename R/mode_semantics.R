rducks_mode_semantics_rows <- list(
  scalar = list(
    ...
    notes = "the ipc (wire) transport covers fixed-width scalars, VARCHAR/BLOB, DECIMAL, INTERVAL, ENUM, BIT, GEOMETRY, MAP, UNION, and LIST/ARRAY/STRUCT of supported types; VARIANT is rejected at registration until the native bridge covers them"
  ),
  vectorized = list(
    mode = "vectorized",
    status = "implemented",
