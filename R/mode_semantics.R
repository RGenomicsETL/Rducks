rducks_mode_semantics_rows <- list(
  scalar = list(
    ...
    notes = "the ipc (wire) transport covers fixed-width scalars, VARCHAR/BLOB, DECIMAL, and INTERVAL; ENUM, bit/geometry/variant, and nested types are rejected at registration until the native bridge covers them"
  ),
  vectorized = list(
    mode = "vectorized",
    status = "implemented",
