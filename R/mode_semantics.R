rducks_mode_semantics_rows <- list(
  scalar = list(
    ...
    notes = "the ipc (wire) transport covers fixed-width scalars, VARCHAR/BLOB, DECIMAL, INTERVAL, ENUM, BIT, GEOMETRY, and LIST/ARRAY/STRUCT of supported types; MAP, UNION, and variant are rejected at registration until the native bridge covers them"
  ),
  vectorized = list(
    mode = "vectorized",
    status = "implemented",
