...
`rducks_value_semantics()` is the package-level schema for row-mode
...
- x:
  ...
  `NULL`, all currently implemented row-mode scalar type semantics are
...
Return semantics are stated from R back to DuckDB. In row mode,
