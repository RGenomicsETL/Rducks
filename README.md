...
lane allowed to call R. With `threads = "single"`, it also sets
`external_threads=1` and `PRAGMA threads=1`; use that registration-safe
setting while adding UDFs.
...
queued backend supports both scalar evaluators: `eval_mode = "R"` and
`eval_mode = "RC"`.
...
it into DuckDB with `rducks_enable()`, and registers scalar-mode R UDFs
with `rducks_register()`. The scalar-mode input/output type set is
