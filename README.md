...
in-process queue. This is a liveness and scheduling feature for
same-process callbacks; it still serializes R API work on the recorded
main R thread.
...
verifies that DuckDB actually uses a non-main worker thread for this
table scan before timing the no-op UDF.
...
vectorized R UDFs with `rducks_register()`.
