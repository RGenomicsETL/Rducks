...
- R-side query consumption: `rducks_query_stream()` exposes explicit
  connection-bound query batches; it is not a SQL UDF registration
  surface.
...
`rducks_enum()`, and `rducks_union()`. Constructed DuckDB type
descriptors use the formal S7 `rducks_type` class and structural
...
batch-by-batch query consumption rather than one eager
`DBI::dbGetQuery()` result. The query runs through the Rducks
extension’s DuckDB streaming result handle; fetched DuckDB chunks are
exported through DuckDB Arrow C Data and materialized with the same
Rducks/nanoarrow conversion helpers used by scalar-UDF marshalling. This
path does not page over an already materialized DBI result and does not
require the heavy `arrow` R package. Because execution uses the
...
stream query scope. Delivery into R is intentionally on the recorded R
thread because the current materializer creates R objects; Rducks does
not call R/nanoarrow materializers from arbitrary DuckDB worker threads.
The returned stream is connection-bound for lifecycle and has
`next_batch()`, `close()`, and `is_closed()` methods. The current
`next_batch()` materializer returns a base R data-frame batch or `NULL`
at end-of-stream. Each non-empty batch carries the stream’s nanoarrow
schema in the `"rducks_nanoarrow_schema"` attribute, and
...
`ipc_globals_share = "mori"` applies `mori::share()` to each selected
