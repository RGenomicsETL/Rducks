...
Rducks/nanoarrow conversion helpers used by scalar-UDF marshalling.
Because execution uses the extension-owned DuckDB connection,
...
is connection-bound for lifecycle and has `next_batch()`, `close()`, and
`is_closed()` methods; `next_batch()` returns a data frame or `NULL` at
end-of-stream. Each non-empty batch carries the stream’s nanoarrow
